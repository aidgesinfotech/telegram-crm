...
exports.dialogs = async (req, res) => {
  try{
    ...
    const fast = await mt.getDialogsFromDB(id, limit, offset);
    // Best-effort attach handler and refresh DB if empty
    try{ await mt.ensureUpdateHandler(id); }catch(_e){}
    if (!fast.total){ try { mt.syncDialogs(id).catch(()=>{}); } catch(_e){} }
    return res.status(200).json({ status: 'success', data: fast.data || [], total: fast.total || 0, cached: true });
  }catch(e){
    res.status(500).json({ error: e.message || 'Internal server error' });
  }
};
