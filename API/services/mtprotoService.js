...
async function getClientForDevice(deviceId){
  ...
  const creating = (async ()=>{
    ...
    let client = new TelegramClient(new StringSession(sessionStr), Number(process.env.API_ID), String(process.env.API_HASH), { connectionRetries: 3 });
    try{
      ...
      if (msg.includes('AUTH_KEY_DUPLICATED')){
        ...
        state.set(deviceId, {});
        client = new TelegramClient(new StringSession(sessionStr), Number(process.env.API_ID), String(process.env.API_HASH), { connectionRetries: 3 });
        await client.connect();
      } else {
        throw e;
      }
